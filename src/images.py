...
class UnicodeReader:
    ...
    def next(self):
        ...
        # delimiter was more than one character (temporal replacements must be
        # reversed)
        if self.replace:
    ...
    def next(self):
        ...
        if self.replace:
            for field, cell in zip(self.fields, row):
                rowdict[field] = unicode(
                    cell.replace(self.mdel, self.big), "utf-8")
        else:
            for field, cell in zip(self.fields, row):
...
def main():
    ...
    with open(fname) as f:
        for line in reader(f, hasheader=True):
            print line
            return
